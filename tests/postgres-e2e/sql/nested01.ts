...
export async function nested01(client: pg.Client | pg.Pool | pg.PoolClient): Promise<Nested01Result[]> {
	const sql = `
	-- @nested
	SELECT
		u.id as user_id,
...
export async function nested01Nested(client: pg.Client | pg.Pool | pg.PoolClient): Promise<Nested01NestedUsers[]> {
